<?php 
    use Core\CSRF;
    $this->setSiteTitle('Edit Article') ?>

<?php $this->start('body') ?>

    <form class="editor shadow" action="<?=PROOT?>post/editArticle/<?=$this->post_id?>" method="post">
        ...
        <div class="input-wrapper">
            ...
            <select name="post_int" class="select">
                ...
                <?php foreach($this->user_interests as $u) : ?>
                    <option <?php if($this->post_int == $u->interest) echo 'selected' ?> > <?= $u->interest ?> </option>
                <?php endforeach ?>
            </select>
        </div>
